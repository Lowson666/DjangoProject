...
def index(request):
    ...
    # 返回渲染模板
    return render(request, 'goods_page/index.html', context)
...
def detail(request,id):
    ...
    response=render(request,'goods_page/detail.html',context)
...
#搜索栏跳转
def search(request):
    goods_name = request.GET.get('searchid')
    goods_id = GoodsInfo.objects.filter(gtitle=goods_name)
    print(goods_name,goods_id)
    print(goods_id[0].id)
    return detail(request,goods_id[0].id)